...
import { AssetDetailSkeleton } from "@/components/detail/AssetDetailSkeleton";
import { PriceChart } from "@/components/detail/PriceChart";
import { CryptoRowSkeletonList } from "@/components/markets/CryptoRowSkeleton";
import { ErrorCallout } from "@/components/ui/ErrorCallout";
import { AnimatedUsdPrice } from "@/components/markets/AnimatedUsdPrice";
...
import { formatCompactNumber, formatSupply, formatUsd } from "@/utils/formatters";
...
export function AssetDetailScreen() {
  ...
  const { fsym, displayName, rank } = route.params as AssetDetailRouteParams;
  const [range, setRange] = useState<HistoryRangeId>("7d");
  const { isFavorite, toggle } = useFavorites();
  const fav = isFavorite(fsym);

  useCryptocomparePriceStream([fsym.trim().toUpperCase()], isFocused);

  const detailQuery = useAssetDetailQuery(fsym, displayName);
  const historyQuery = useAssetHistoryQuery(fsym, range);
  ...
  }, [navigation, fsym, displayName, fav, toggle]);
